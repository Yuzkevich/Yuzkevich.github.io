const   workers = document.querySelectorAll('.workers__wraper .worker'),
        ...
        alertInner = document.querySelector('.alert__innner'),
        inputPassword = document.querySelector('.password'),
        inAdmin = document.querySelector('.in'),
        cancelAdmin = document.querySelector('.cancel'),
        alertDescr = document.querySelector('.alert__descr'),
        password = '2596',
        workersWraper = document.querySelector('.workers__wraper'),
        navWraper = document.querySelector('.nav__wraper'),
        clear = document.querySelector('.clear'),
        okModal = document.querySelector('.ok'),
...
inputPassword.addEventListener('input', () => {
    xxxPassword += `${inputPassword.value.split('').reverse().join('')[0]}`;
    let icon = '';
    for (let i = 0; i < inputPassword.value.length; i++) {
        icon += '*';
    }
    inputPassword.value = icon;
});

inAdmin.addEventListener('click', () => {
    if (xxxPassword === password) {
        xxxPassword = '';
        inputPassword.value = '';
        workersWraper.style.display = 'block';
        navWraper.style.display = 'block';
        alertInner.style.display = 'none';
        document.querySelectorAll('.edit').forEach(item => {
            item.style.display = 'inline-block';
        });
        document.querySelector('.editt').style.display = 'inline-block';
        document.querySelector('.zp__wraper').style.marginLeft = '240px';
